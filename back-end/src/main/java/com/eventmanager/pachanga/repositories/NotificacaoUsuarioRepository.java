...
public interface NotificacaoUsuarioRepository extends JpaRepository<NotificacaoUsuario, Integer>{
	...
	@Query(value = "SELECT nu FROM NotificacaoUsuario nu JOIN nu.usuario u WHERE u.codUsuario = :idUser AND nu.codNotificacaoUsuario = :codNotificacao AND nu.mensagem = :mensagem")
